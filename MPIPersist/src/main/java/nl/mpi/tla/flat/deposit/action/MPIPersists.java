                    ...
                    while (target.exists()) {
                        if (res.isUpdate()) {
                            ...
                            if (oldFile.renameTo(oldFileRename)) {
                                ...
                                context.registerRollbackEvent(this, "modifyDatastream", "fid", fid, "dsid", dsid);
                                ...
                                context.registerRollbackEvent(this, "updateLocation", "oldPath", "file:/" + oldFile.getPath(), "fid", fid, "dsid", dsid);
            ...
            try {
                ...
                if (tpe.equals("move")) {
                    ...
                    if (to.exists() && to.isFile() && to.canWrite() && !from.exists()) {
                        ...
                        logger.debug("rollback action[" + this.getName() + "] event[" + tpe + "] moved [" + to
                                + "] back to [" + from + "]");
                    } else {
                        logger.error("rollback action[" + this.getName() + "] event["
                                + Saxon.xpath2string(event, "@type") + "] failed source[" + from + "] and destination["
                                + to + "] can't be restored!");
                    }
